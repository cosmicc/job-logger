...
def test_authenticated_help_page_renders_version_and_changelog(
    ...
    assert ">v2.0.1<" in response.text
    assert '<p class="help-version-release-date">Released: 07.30.2026</p>' in response.text
    assert "Released: 07.30.2026" in response.text
...
def test_help_page_marks_dev_build(authenticated_client: TestClient) -> None:
    ...
    assert response.status_code == 200
    assert ">v2.0.1 DEV<" in response.text
