...
function minimumRoundedStopForDisplay(displayElement) {
  ...
  const configuredMinimumMinutes = Number.parseInt(displayElement.dataset.minimumDurationMinutes || "", 10);
  const minimumDurationMinutes = Number.isFinite(configuredMinimumMinutes)
    ? configuredMinimumMinutes
    : ROUNDING_INTERVAL_MINUTES;
  return new Date(roundedStartDate.getTime() + minimumDurationMinutes * 60 * 1000);
}
...
function updateActiveTimeDisplays(activeTimeForm, payload) {
  ...
  if (stopTimeInput && payload.rounded_stop_time) {
    ...
    if (payload.minimum_duration_minutes) {
      stopTimeInput.dataset.minimumDurationMinutes = String(payload.minimum_duration_minutes);
    }
  }
...
function openServiceCallCalendar(panel) {
  ...
  if (window.TicketPilotDateTimeControls && typeof window.TicketPilotDateTimeControls.openDateChooser === "function") {
    window.TicketPilotDateTimeControls.openDateChooser(dateInput);
    return;
  }
  ...
  serviceCallForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (
        ...
        && window.TicketPilotNavigation
        && window.TicketPilotNavigation.launch(
...
function updateActiveTicketDisplay(jobId, selectedTicket) {
  ...
  const ticketHeadings = findDisplayElements("[data-active-ticket-heading]");
  ...
  for (const ticketHeading of ticketHeadings) {
  ...
  if (ticketTimeEntriesButtons.length && window.TicketPilotTicketNotes) {
    window.TicketPilotTicketNotes.refreshTimeEntriesButton(ticketTimeEntriesButtons[0]);
  }
  const ticketNavigationButton = activeJobCard.querySelector("[data-ticket-navigation-button]");
  if (ticketNavigationButton && window.TicketPilotNavigation) {
    window.TicketPilotNavigation.applyDestination(
