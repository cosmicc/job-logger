const TICKET_PILOT_TIME_STEP_MINUTES = 15;
const TICKET_PILOT_MINUTES_PER_DAY = 24 * 60;

let activeDateChooserState = null;
let activeTimeDropdownState = null;

function ticketPilotDateTimePadTwo(value) {
  return String(value).padStart(2, "0");
}

function ticketPilotDateTimeSafeString(value) {
  return String(value || "");
}

function ticketPilotNormalizedDateValue(dateValue) {
  const normalizedDate = ticketPilotDateTimeSafeString(dateValue).trim();
...
function ticketPilotDateValueFromDate(dateValue) {
  const year = String(dateValue.getFullYear()).padStart(4, "0");
  const month = ticketPilotDateTimePadTwo(dateValue.getMonth() + 1);
  const day = ticketPilotDateTimePadTwo(dateValue.getDate());
  return `${year}-${month}-${day}`;
}

function ticketPilotCurrentDetroitDateValue() {
  ...
  if (!partValues.year || !partValues.month || !partValues.day) {
    return ticketPilotDateValueFromDate(new Date());
  }

  return `${partValues.year}-${partValues.month}-${partValues.day}`;
}

function ticketPilotParseTimeToMinutes(timeValue) {
  const normalizedTimeValue = ticketPilotDateTimeSafeString(timeValue).trim().toLowerCase().replace(/\s+/g, " ");
...
function ticketPilotFormatMinutesAsTime(totalMinutes) {
  const normalizedTotalMinutes = (
    (totalMinutes % TICKET_PILOT_MINUTES_PER_DAY) + TICKET_PILOT_MINUTES_PER_DAY
  ) % TICKET_PILOT_MINUTES_PER_DAY;
  const hour24 = Math.floor(normalizedTotalMinutes / 60);
  const minute = normalizedTotalMinutes % 60;
  const hour12 = hour24 % 12 || 12;
  const period = hour24 < 12 ? "am" : "pm";
  return `${hour12}:${ticketPilotDateTimePadTwo(minute)} ${period}`;
}

function ticketPilotCommitInputValue(inputElement, value) {
  inputElement.value = value;
  inputElement.dispatchEvent(new Event("input", {bubbles: true}));
  inputElement.dispatchEvent(new Event("change", {bubbles: true}));
}

function ticketPilotPositionFloatingElement(anchorElement, floatingElement) {
...
function ticketPilotCloseDateChooser({restoreFocus = false} = {}) {
...
function ticketPilotRenderDateChooser() {
  if (!activeDateChooserState) {
    return;
  }
  ...
  const todayValue = ticketPilotCurrentDetroitDateValue();
  ...
  for (let dayOffset = 0; dayOffset < 42; dayOffset += 1) {
    ...
    const dayDateValue = ticketPilotDateValueFromDate(dayDate);
  ...
  todayButton.addEventListener("click", () => {
    const currentTodayValue = ticketPilotCurrentDetroitDateValue();
    ticketPilotCommitInputValue(inputElement, currentTodayValue);
    ticketPilotCloseDateChooser({restoreFocus: true});
  });
  ...
  setButton.addEventListener("click", () => {
    ticketPilotCommitInputValue(inputElement, activeDateChooserState.selectedDateValue);
    ticketPilotCloseDateChooser({restoreFocus: true});
  });
  ...
  ticketPilotPositionFloatingElement(inputElement, popoverElement);
...
function ticketPilotOpenDateChooser(inputElement) {
  if (!inputElement || inputElement.disabled) {
    return;
  }

  ticketPilotCloseTimeDropdown();
  ticketPilotCloseDateChooser();

  const currentDateValue = ticketPilotNormalizedDateValue(inputElement.value)
    ? inputElement.value
    : ticketPilotCurrentDetroitDateValue();
  const currentDateInfo = ticketPilotNormalizedDateValue(currentDateValue);
  if (!currentDateInfo) {
    return;
  }
...
function ticketPilotCloseTimeDropdown({restoreFocus = false} = {}) {
...
function ticketPilotOpenTimeDropdown(inputElement) {
  ...
  const selectedMinutes = ticketPilotParseTimeToMinutes(inputElement.value);
  const roundedSelectedMinutes = selectedMinutes === null
    ? 0
    : Math.round(selectedMinutes / TICKET_PILOT_TIME_STEP_MINUTES) * TICKET_PILOT_TIME_STEP_MINUTES;
  const normalizedSelectedMinutes = (
    (roundedSelectedMinutes % TICKET_PILOT_MINUTES_PER_DAY) + TICKET_PILOT_MINUTES_PER_DAY
  ) % TICKET_PILOT_MINUTES_PER_DAY;
  ...
  for (let minuteValue = 0; minuteValue < TICKET_PILOT_MINUTES_PER_DAY; minuteValue += TICKET_PILOT_TIME_STEP_MINUTES) {
    const optionButton = document.createElement("button");
    optionButton.type = "button";
    optionButton.className = "time-dropdown-option";
    optionButton.setAttribute("role", "option");
    optionButton.dataset.timeMinutes = String(minuteValue);
    optionButton.textContent = ticketPilotFormatMinutesAsTime(minuteValue);
    ...
    optionButton.addEventListener("click", () => {
      ticketPilotCommitInputValue(inputElement, ticketPilotFormatMinutesAsTime(minuteValue));
      ticketPilotCloseTimeDropdown({restoreFocus: true});
    });
  ...
  inputElement.setAttribute("aria-expanded", "true");
  ticketPilotPositionFloatingElement(inputElement, dropdownElement);
  if (selectedButton) {
    selectedButton.scrollIntoView({block: "center"});
  }
}

function ticketPilotInitializeDateInputs(root = document) {
  ...
  for (const dateInput of dateInputs) {
    ...
    dateInput.addEventListener("click", (event) => {
      event.preventDefault();
      ticketPilotOpenDateChooser(dateInput);
    });
...
function ticketPilotInitializeTimeInputs(root = document) {
  ...
  for (const timeInput of timeInputs) {
    ...
    timeInput.addEventListener("focus", () => {
      ticketPilotOpenTimeDropdown(timeInput);
    });
...
function ticketPilotInitializeDateTimeControls(root = document) {
  ticketPilotInitializeDateInputs(root);
  ticketPilotInitializeTimeInputs(root);
}
...
window.addEventListener("resize", () => {
  if (activeDateChooserState) {
    ticketPilotPositionFloatingElement(activeDateChooserState.inputElement, activeDateChooserState.popoverElement);
  }
  if (activeTimeDropdownState) {
    ticketPilotPositionFloatingElement(activeTimeDropdownState.inputElement, activeTimeDropdownState.dropdownElement);
  }
});

window.TicketPilotDateTimeControls = {
  initialize: ticketPilotInitializeDateTimeControls,
  openDateChooser: ticketPilotOpenDateChooser,
  openTimeDropdown: ticketPilotOpenTimeDropdown,
};

if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => ticketPilotInitializeDateTimeControls(), {once: true});
} else {
  ticketPilotInitializeDateTimeControls();
}
