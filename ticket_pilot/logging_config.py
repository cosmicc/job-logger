"""Runtime stdout logging configuration for TicketPilot."""
...
from ticket_pilot.time_utils import to_local
...
def configure_logging(application_settings: Settings) -> None:
    ...
    for handler in list(root_logger.handlers):
        if getattr(handler, "_ticket_pilot_marker", "") == "ticket_pilot_stdout":
            stdout_handler = handler
            continue
        if getattr(handler, "_ticket_pilot_marker", "") == "ticket_pilot_app_file":
    ...
    if stdout_handler is not None:
        ...
        stream_handler._ticket_pilot_marker = "ticket_pilot_stdout"  # type: ignore[attr-defined]
