...
from job_logger.services.pushover import send_pushover_notification
...
def health_snapshot_message(snapshot: AppHealthSnapshot) -> str:
    ...
    if not snapshot.issues:
        return "All monitored Job Logger checks are passing."
...
def notify_if_health_changed(
    ...
    *,
    ...
    if current_fingerprint:
        title = "Job Logger health degraded"
        if previous_fingerprint:
            title = "Job Logger health changed"
    ...
    send_pushover_notification(
        ...
        "All monitored Job Logger checks are passing again.",
        priority=0,
        application_settings=application_settings,
    )
    return "restored"
